    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-end z-50 overflow-hidden">
      <div className="w-[600px] h-full bg-white flex flex-col">
        ...
        <div className="flex justify-between items-center px-6 py-4 h-[60px]">
          <h2 className="text-base font-medium text-[#2B2834] leading-[19px] tracking-[-0.01em]">User Details</h2>
          <button 
            onClick={onClose}
            className="flex items-center gap-1.5 px-2 py-1.5 bg-[#F9F9FB] rounded-md h-7"
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M4 12L11.96 4M12 12L4.04 4" stroke="#5F2EFC" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span className="text-xs font-medium text-[#5F2EFC]">Close</span>
          </button>
        </div>

        {/* Divider */}
        <div className="w-full h-px bg-[#F0EBF4]" />
        ...
        <div className="flex-1 flex flex-col overflow-y-auto">
          ...
          <div className="flex flex-col px-6 py-6 gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 flex flex-col justify-center gap-0.5">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 bg-[#E7F3EF] rounded text-[10px] font-medium text-[#239B73]">
                ...
                <div className="flex flex-col">
                  <span className="text-base font-bold text-[#2B2834] leading-[19px]">{user.name}</span>
                  <span className="text-[11px] italic text-[#A5A1AF] leading-[13px]">@username</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button className="flex items-center gap-1 px-4 py-3.5 bg-white border border-[#EBEBEB] rounded-[9px] h-8 shadow-[inset_0px_1.5px_1px_rgba(255,255,255,0.11)]">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1.57129 5.14267L6.22462 8.24533C6.86662 8.67267 7.18729 8.88667 7.53396 8.97C7.84062 9.04333 8.15996 9.04333 8.46596 8.97C8.81262 8.88667 9.13329 8.67267 9.77529 8.24533L14.4286 5.14267M4.77129 13H11.2286C12.3486 13 12.9086 13 13.3366 12.782C13.7127 12.5901 14.0184 12.2842 14.21 11.908C14.4286 11.48 14.4286 10.92 14.4286 9.8V6.2C14.4286 5.08 14.4286 4.52 14.2106 4.092C14.0189 3.71569 13.7129 3.40974 13.3366 3.218C12.9086 3 12.3486 3 11.2286 3H4.77129C3.65129 3 3.09129 3 2.66329 3.218C2.28723 3.40986 1.98153 3.71579 1.78996 4.092C1.57129 4.52 1.57129 5.08 1.57129 6.2V9.8C1.57129 10.92 1.57129 11.48 1.78929 11.908C1.98103 12.2843 2.28698 12.5903 2.66329 12.782C3.09129 13 3.65129 13 4.77129 13Z" stroke="#5F5971" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span className="text-[13.5px] font-medium text-[#353A44] leading-4">Send email</span>
                </button>
                <button className="flex items-center justify-center w-8 h-8 bg-white border border-[#EBEBEB] rounded-[9px] shadow-[inset_0px_1.5px_1px_rgba(255,255,255,0.11)]">
                  <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <circle cx="10" cy="10" r="1.5" fill="#121212"/>
                    <circle cx="4" cy="10" r="1.5" fill="#121212"/>
          ...
          <div className="flex w-full h-[38px]">
            ...
            >
              <span className={`text-[13.5px] font-medium leading-4 ${activeTab === 'profile' ? 'text-[#2B2834]' : 'text-[#A5A1AF]'}`}>Profile</span>
              <div className={`w-full h-0.5 ${activeTab === 'profile' ? 'bg-[#5F2EFC] border-2 border-[#5F2EFC]' : 'bg-[#EBEBEB] border border-[#EBEBEB]'}`} />
            </button>
            <button 
              onClick={() => setActiveTab('activity')}
              className="flex-1 flex flex-col justify-center items-center pt-2.5 gap-3"
            >
              <span className={`text-[13.5px] font-medium leading-4 ${activeTab === 'activity' ? 'text-[#2B2834]' : 'text-[#A5A1AF]'}`}>Activity</span>
              <div className={`w-full h-0.5 ${activeTab === 'activity' ? 'bg-[#5F2EFC] border-2 border-[#5F2EFC]' : 'bg-[#EBEBEB] border border-[#EBEBEB]'}`} />
            </button>
            <button 
              onClick={() => setActiveTab('purchases')}
              className="flex-1 flex flex-col justify-center items-center pt-2.5 gap-3"
            >
              <span className={`text-[13.5px] font-medium leading-4 ${activeTab === 'purchases' ? 'text-[#2B2834]' : 'text-[#A5A1AF]'}`}>Purchases</span>
              <div className={`w-full h-0.5 ${activeTab === 'purchases' ? 'bg-[#5F2EFC] border-2 border-[#5F2EFC]' : 'bg-[#EBEBEB] border border-[#EBEBEB]'}`} />
            </button>
            <button 
              onClick={() => setActiveTab('subscriptions')}
              className="flex-1 flex flex-col justify-center items-center pt-2.5 gap-3"
            >
              <span className={`text-[13.5px] font-medium leading-4 ${activeTab === 'subscriptions' ? 'text-[#2B2834]' : 'text-[#A5A1AF]'}`}>Subscriptions</span>
              <div className={`w-full h-0.5 ${activeTab === 'subscriptions' ? 'bg-[#5F2EFC] border-2 border-[#5F2EFC]' : 'bg-[#EBEBEB] border border-[#EBEBEB]'}`} />
            </button>
          </div>

          {/* Tab Content */}
          {activeTab === 'profile' && (
            <div className="flex flex-col px-6 py-6 gap-3 bg-[#F9F9FB] flex-1">
              <div className="flex flex-col gap-1">
                <ProfileField icon="user" label="First Name" value="Noah" />
                <ProfileField icon="user" label="Last Name" value="Smith" />
                <ProfileField icon="user" label="User Name" value="@noahsmith" />
                <ProfileField icon="mail" label="Email Address" value="Noahsmith@email.com" />
                <ProfileField icon="calendar" label="Date of Birth" value="08/06/1990" />
                <ProfileField icon="gender" label="Gender" value="Male" />
                <ProfileField icon="arrow" label="Referred By" value="Damson Ben" link />
                <ProfileField icon="id" label="Creator Profile" value="Temilade Odunsi" link />
              </div>
            </div>
          )}

          {activeTab === 'activity' && (
            <div className="flex flex-col px-6 pt-4 pb-6 gap-1 bg-[#F9F9FB] flex-1">
              {/* Table Header */}
              <div className="flex items-center px-4 gap-6 h-8 relative z-10">
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 w-[141.4px]">Timestamp</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Activity</span>
              </div>

              {/* Activity Table */}
              <div className="flex flex-col bg-white border border-[#EBEBEB] rounded-lg overflow-hidden relative z-0">
                <ActivityRow timestamp="2 minutes ago" activity='Joined "Skit Makers NG" chat' />
                <ActivityRow timestamp="10 minutes ago" activity="Attended Live Session (67 viewers)" />
                <ActivityRow timestamp="2 hours ago" activity="Purchased Paid Call" />
                <ActivityRow timestamp="12 hours ago" activity="Posted in Hub (+12 reactions)" />
                <ActivityRow timestamp="22 hours ago" activity="Posted in Hub (+12 reactions)" />
                <ActivityRow timestamp="3 days ago" activity="Posted in Hub (+12 reactions)" last />
              </div>
            </div>
          )}
          ...
          {activeTab === 'purchases' && (
            <div className="flex flex-col px-6 pt-4 pb-6 gap-1 bg-[#F9F9FB] flex-1">
              {/* Table Header */}
              <div className="flex items-center px-4 gap-6 h-8 relative z-10">
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 w-[64.3px]">Date</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Product</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Creator</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Amount</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 w-[50px]">Status</span>
              </div>

              {/* Purchases Table */}
              <div className="flex flex-col bg-white border border-[#EBEBEB] rounded-lg overflow-hidden relative z-0">
                <PurchaseRow date="4/4/18" product="Paid Call" creator="@rileybrown" amount="₦2,321.53" status="paid" />
                <PurchaseRow date="9/4/12" product="Paid Call" creator="@chloegrace" amount="₦4,231.01" status="failed" />
                <PurchaseRow date="10/28/12" product="Paid Call" creator="@gavinjones" amount="₦48,013.11" status="paid" />
                <PurchaseRow date="1/31/14" product="Course" creator="@ellierose" amount="₦5,932.32" status="paid" />
                <PurchaseRow date="2/11/12" product="Course" creator="@noahsmith" amount="₦10,075.14" status="paid" />
                <PurchaseRow date="3/4/16" product="Course" creator="@zoeywilson" amount="₦7,231.85" status="paid" last />
              </div>
            </div>
          )}
          ...
          {activeTab === 'subscriptions' && (
            <div className="flex flex-col px-6 pt-4 pb-6 gap-1 bg-[#F9F9FB] flex-1">
              ...
              <div className="flex items-center px-4 gap-6 h-8 relative z-10">
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 w-[64.3px]">Date</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Product/Service</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Creator</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 flex-1">Amount</span>
                <span className="text-[13.5px] font-medium text-[#5F5971] leading-4 w-[50px]">Status</span>
              </div>
              ...
              {/* Subscriptions Table */}
              <div className="flex flex-col bg-white border border-[#EBEBEB] rounded-lg overflow-hidden relative z-0">
                <PurchaseRow date="2 min ago" product="Paid Call" creator="@rileybrown" amount="₦2,321.53" status="paid" />
                <PurchaseRow date="3 min ago" product="Paid Call" creator="@chloegrace" amount="₦4,231.01" status="failed" />
                <PurchaseRow date="5 min ago" product="Paid Call" creator="@gavinjones" amount="₦48,013.11" status="paid" />
                <PurchaseRow date="11 min ago" product="Course" creator="@ellierose" amount="₦5,932.32" status="paid" />
                <PurchaseRow date="14 min ago" product="Course" creator="@noahsmith" amount="₦10,075.14" status="paid" />
                <PurchaseRow date="23 min ago" product="Course" creator="@zoeywilson" amount="₦7,231.85" status="paid" last />
              </div>
            </div>
          )}
        </div>
...
function PurchaseRow({ date, product, creator, amount, status, last }: { date: string; product: string; creator: string; amount: string; status: 'paid' | 'failed'; last?: boolean }) {
  const statusConfig = {
    paid: {
      bg: '#E7F3EF',
      color: '#239B73',
  ...
  return (
    <div className={`flex items-center px-4 gap-6 h-8 bg-white ${!last ? 'border-b border-[#EBEBEB]' : ''}`}>
      <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 w-[64.3px]">{date}</span>
      <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 flex-1">{product}</span>
      <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 flex-1">{creator}</span>
      <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 flex-1">{amount}</span>
...
function ActivityRow({ timestamp, activity, last }: { timestamp: string; activity: string; last?: boolean }) {
  return (
    <div className={`flex items-center px-4 gap-6 h-8 bg-white ${!last ? 'border-b border-[#EBEBEB]' : ''}`}>
      <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 w-[141.4px]">{timestamp}</span>
      <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 flex-1">{activity}</span>
    </div>
  );
}
...
function ProfileField({ icon, label, value, link }: { icon: string; label: string; value: string; link?: boolean }) {
  return (
    <div className="flex items-center px-4 gap-4 h-8 bg-white border border-[#EBEBEB] rounded">
      <div className="flex items-center gap-2 w-[97px]">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          {icon === 'user' && (
            <>
              <circle cx="8" cy="5" r="2.5" stroke="#5F5971" strokeWidth="1"/>
              <path d="M3.5 13C3.5 10.5 5.5 8.5 8 8.5C10.5 8.5 12.5 10.5 12.5 13" stroke="#5F5971" strokeWidth="1"/>
            </>
          )}
          {icon === 'mail' && (
            <>
        ...
        <span className="text-xs text-[#5F5971] leading-[14px]">{label}</span>
      </div>
      <div className="w-px h-8 bg-[#EBEBEB]" />
      {link ? (
        <div className="flex items-center gap-1 flex-1">
          <span className="text-[13.5px] font-medium text-[#5F2EFC] leading-4 underline">{value}</span>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            <path d="M2 12L12 2M12 2H5M12 2V9" stroke="#5F2EFC" strokeWidth="1"/>
          </svg>
        </div>
      ) : (
        <span className="text-[13.5px] font-medium text-[#2B2834] leading-4 flex-1">{value}</span>
      )}
    </div>
  );
}
