...
const StatPill = ({ label, value }: { label: string; value: string }) => (
  <span className="flex-1 inline-flex items-center justify-between h-7 p-2 rounded bg-surface-secondary">
    <span className="h-3 font-normal text-[10px] leading-3 tracking-normal text-[#676767] font-sans">{label}</span>
    <span className="font-medium text-[10px] leading-3 tracking-normal text-text-primary font-sans">{value}</span>
  </span>
);
...
const ActivityCard = ({ title, value, children }: { title: string; value: string; children: React.ReactNode }) => (
  <div className="bg-white flex-1 h-24 p-1 flex flex-col rounded-lg border border-border-primary box-border">
    <div className="p-2.5 flex flex-col gap-1">
      <p className="w-full h-[14px] font-normal text-xs leading-[14px] text-text-secondary font-sans">{title}</p>
      <p className="w-full h-[22px] font-medium text-lg leading-[22px] tracking-normal text-text-primary font-sans">{value}</p>
    </div>
...
export default function ActivityOverview() {
  return (
    <section className="w-full flex flex-col gap-2">
      <h2 className="w-full h-6 font-bold text-xl leading-6 -tracking-[0.01em] text-text-primary font-sans">Activity Overview</h2>
