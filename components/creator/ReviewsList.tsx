...
const ReviewItem = ({ review, isLast }: { review: Review; isLast?: boolean }) => (
  <div className={`flex justify-center items-start py-6 gap-8 w-full bg-white ${!isLast ? 'border-b border-border-primary' : ''}`}>
    {/* User Info */}
...
export default function ReviewsList({
  ...
  return (
    <div className={`flex flex-col justify-center items-center gap-2 isolate w-full ${className}`}>
      {/* Header */}
      <div className="flex justify-center items-center gap-2.5 w-full h-[30px] z-[1]">
        <h2 className="text-base font-medium leading-none text-text-primary font-sans h-[19px] flex-1 tracking-[0%]">
          {title}
        </h2>
        <TimeDropdown
          options={timeOptions}
      ...
      <div className="flex flex-col items-start p-1 gap-1 w-full rounded-lg z-0">
        {/* Stats Section */}
        {showStats && (
          <div className="flex items-center w-full h-[100px] bg-white border border-border-primary rounded">
            {/* Total Reviews */}
            <div className="flex flex-col items-start px-[105px] py-4 gap-1 w-[348.67px] h-[74px] border-r border-border-primary flex-1">
              <span className="text-xs font-normal leading-[14px] text-text-secondary font-sans w-[78px]">
                Total Reviews
              </span>
            ...
            <div className="flex flex-col items-start px-[105px] py-4 gap-1 w-[348.67px] h-[74px] border-r border-border-primary flex-1">
              ...
              <div className="flex justify-center items-center gap-2.5 w-[134px] h-6">
                <span className="text-xl font-medium leading-6 tracking-[-0.01em] text-text-primary font-sans w-7">
            ...
            {/* Rating Distribution */}
            <div className="flex flex-col justify-center items-start px-[105px] py-4 gap-0.5 w-[348.67px] h-[100px] border-r border-border-primary flex-1">
              {ratingDistribution.map((item, index) => (
                <div key={item.stars} className="flex items-center gap-[5px] w-full h-3">
                  ...
                  <div className="flex items-start gap-0 w-6 h-3 -ml-px">
                    ...
                    <span className="text-[10px] font-medium leading-3 text-center text-text-secondary font-sans flex-1 w-[13px]">
                      {item.stars}
                    </span>
                  </div>
                  ...
                  {/* Count */}
                  <span className="text-[10px] font-medium leading-3 text-text-primary font-sans flex-none">
                    {item.count}
                  </span>
                </div>
              ))}
        ...
        {/* Reviews List */}
        <div className="flex flex-col items-center px-16 py-6 w-full bg-white border border-border-primary rounded">
